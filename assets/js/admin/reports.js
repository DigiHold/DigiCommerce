(() => {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function formatCurrency(amount) {
      ...
      return digicommerceReports.currency_position === "left" ? `${symbol}${number}` : `${number}${symbol}`;
    }
    function updateStatCard(selector, value) {
      const element = document.querySelector(selector);
      if (element) {
        element.textContent = typeof value === "number" && !selector.includes("orders") ? formatCurrency(value) : value.toLocaleString();
      }
    }
    function showLoadingState() {
      ...
      if (currentTab === "overview") {
        document.querySelectorAll(".stat-card").forEach((card) => {
          ...
          if (amount)
            amount.textContent = digicommerceReports.i18n.loading;
        });
      } else if (tabContent) {
      ...
      const chartConfig = {
        ...
        data: {
          ...
          datasets: [{
            ...
            data: data.revenue,
            borderColor: getComputedStyle(document.documentElement).getPropertyValue("--dc-gold").trim(),
            backgroundColor: getComputedStyle(document.documentElement).getPropertyValue("--dc-gold").trim() + "20",
            tension: 0.4,
            fill: true
          }]
        },
